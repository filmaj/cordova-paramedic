...
ParamedicRunner.prototype.runAppiumTests = function (useSauce) {
    ...
    var options = {
        ...
        appiumPlatformVersion: self.config.getSaucePlatformVersion(),
...
ParamedicRunner.prototype.runTests = function () {
    ...
    if (this.config.shouldUseSauce()) {
        ...
        return this.setLocalTestTarget()
        .then(function () {
            return self.runLocalTests();
        })
        .then(function (result) {
            isTestPassed = result;
        })
...
ParamedicRunner.prototype.setLocalTestTarget = function () {
    var self = this;
    ...
    return Q().then(function () {
        ...
        return paramedicTargetChooser.chooseTarget(/*useEmulator=*/true, /*preferredTarget=*/configTarget, self.config.getSaucePlatformVersion());
    })
    .then(function(targetObj){
        logger.normal('Setting local test target to: ' + JSON.stringify(targetObj));
