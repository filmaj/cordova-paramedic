...
function getSimulatorModelId(target, version) {
    var findSimCommand = 'cordova run --list --emulator | grep ';
    if (target) {
        findSimCommand += '"' + target + '" | ' + (version ? 'grep "' + version + '" | ' : '');
    } else {
        findSimCommand = ' ^iPhone | ' + (version ? 'grep "' + version + '" | ' : '');
    }
    findSimCommand += 'tail -n1';
